...
download_artifact() {
    ...
    if [ "$t" = "jar" ]; then
        if download_file "$MIRROR/$p/$a/$v/$a-$v.jar" "$d/$a-$v.jar"; then
            downloaded_something=1
        fi
    fi

    if [ "$downloaded_something" = "1" ] && [ -f "$d/$a-$v.pom" ]; then
...
resolve_parent_poms() {
    ...
    parent_g=$(grep -oP '(?<=<groupId>)[^<]+' "$pom_file" | head -1 || true)
    parent_a=$(grep -oP '(?<=<artifactId>)[^<]+' "$pom_file" | head -1 || true)
    parent_v=$(grep -oP '(?<=<version>)[^<]+' "$pom_file" | head -1 || true)

    local in_parent=0
    local found_parent=0
    while IFS= read -r line; do
        ...
        if [ $in_parent -eq 1 ]; then
            if echo "$line" | grep -qP '<groupId>'; then
                parent_g=$(echo "$line" | grep -oP '(?<=<groupId>)[^<]+')
            elif echo "$line" | grep -qP '<artifactId>'; then
                parent_a=$(echo "$line" | grep -oP '(?<=<artifactId>)[^<]+')
            elif echo "$line" | grep -qP '<version>'; then
                parent_v=$(echo "$line" | grep -oP '(?<=<version>)[^<]+')
            fi
        fi
    done < "$pom_file"
...
echo ""
echo "=== Phase 2: Iterative resolution with parent POM chasing ==="
MAX_ITERATIONS=200
for i in $(seq 1 $MAX_ITERATIONS); do
    ...
    if [ -z "$missing" ]; then
        echo "No extractable missing from compile. Trying dependency:resolve..."
