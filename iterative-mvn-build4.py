...
def extract_missing(output):
    ...
    # All patterns
    patterns = [
        r'could not be resolved:\s+([^\s:]+):([^\s:]+):([^\s:]+):([^\s:]+)',
        r'artifact\s+([^\s:]+):([^\s:]+):([^\s:]+):([^\s:]+)\s+has not been downloaded',
        r'POM for\s+([^\s:]+):([^\s:]+):([^\s:]+):([^\s:]+)\s+is missing',
        r'Failed to read artifact descriptor for\s+([^\s:]+):([^\s:]+):([^\s:]+):([^\s:]+)',
    ]
...
for iteration in range(1, MAX_ITERATIONS + 1):
    ...
    # Try with -X for deeper error info
    missing = extract_missing(output)
    
    if not missing:
        # Try debug mode
        result2 = subprocess.run(
            ["mvn", "compile", "-DskipTests", "-o", "-X"],
            capture_output=True, text=True, cwd="/workspace/spot-the-difference-server"
        )
        missing = extract_missing(result2.stdout + result2.stderr)
